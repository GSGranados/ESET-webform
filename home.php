<?php
	 ...
	 //requiriendo paths y procedimientos comunes a ocupar
	...
	if ($date>$dateEnd){
		...
		$productocode=0;$essforeis='0';
			...
			if ($license_type_id==1 || $license_type_id==2 || $license_type_id==3 || $license_type_id==9 || $license_type_id==12){
				...
				//Validacion que los datos sean correctos y que la licencia este activa, pais sea correcto apra renovacion
				  if (($productocode > 0) && ($license_state=='Active') && ($go==1)){$mensaje = "Proceso de Renovacion";
		 ...
		 //Licencias que no aplica para renovacion segun no cumple quesitos de sistema
			if($license_state!='Active'){$mensaje2 = " Licencia ".$licenseusername." <strong>Inactiva</strong>";}
			else{$mensaje2 = " Licencias no aplica a renovación";}
		 
		//Match de respuestas si no cumple (orden gerarquico) licencias inactivas o canceladas, si no cumple pais para renovacion y mensaje de error estandar
		 ...
		 if($go=='0'){
			 echo "
				...
				<div class='panel panel-default' >
  					<div class='panel-heading'>
    					<h3 class='panel-title'>País de licencia invalido para renovación</h3>
  					</div>
  					<div class='panel-body'>
						<div class='product-price'>
							<blockquote>
								<p>Validar los datos ingresados o usar una de las siguientes opciones:</p>
								<footer><p>Adquirir Licencia Nueva:<br><a class='btn btn-primary' href='../index.php'>Comprar</a></p>
  								<p>Solicitar Asistencia:<br><a class='btn btn-primary' href='https://www.esetca.com/soporte/contact.php'>Crear Ticket Soporte</a></p></footer>				
							</blockquote>
						</div><!-- Product Single - Price End -->
					</div>
				</div>
		 ...
		 //datos en array, disponible documentacion en pag.57 del manual API 
	    } else {$control = 'Ocurrio un problema consultando el licenciamiento '.$siat17e;
				echo "
				<br>
				<div class='panel panel-default' >
  					<div class='panel-heading'>
		   ...
		   $control = 'Ocurrio un problema consultando el licenciamiento '.$siat17e;
				echo "
				<br>
				<div class='panel panel-default' >
  					<div class='panel-heading'>
